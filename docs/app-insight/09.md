...
| ![](../Images/tip.png) | 提示:在前面的清单中，我们使用了 CreateDefault 初始值设定项(从 ApplicationInsights.config 参数开始创建一个实例)，因为要使用默认构造函数手工初始化的属性列表很长。 |
...
| ![](../Images/note.png) | 注意:我们将使用 ASP.NET MVC 网络应用程序和 Visual Studio 创建这个例子。对于不同的语言和 IDEs，可能有一种等效的方法。 |
