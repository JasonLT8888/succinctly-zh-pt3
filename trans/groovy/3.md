...
![](../images/00011.jpeg)注意:没有声明自己在包中的代码(比如我们的脚本)被分配给默认包。
...
![](../images/00012.jpeg)提示:[org . codehaus . groovy . runtime](https://docs.oracle.com/javase/8/docs/api/java/util/stream/Collector.html)包中的几个类向 Java 类中添加了许多方法。当您不确定某个方法来自哪里时，查看诸如 StringGroovyMethods、DefaultGroovyMethods 和 typehandling 子包等类的文档会有所帮助。
...
![](../images/00013.jpeg)注意:注释在代码中执行各种功能。调用注释时使用“at”符号(@)。
...
![](../images/00012.jpeg)显式导入那些默认项目没有坏处，但是随着时间的推移，你会发现你只是把它们当成理所当然。
...
![](../images/00012.jpeg)提示:当您开始编写自己的类时，您会发现 Groovy 可以自动提供 getters 和 setters，这将从大量的样板文件中节省您的代码。
...
![](../images/00014.jpeg)提示:这适用于 XHTML 页面，但是如果您想解析 HTML 页面，请查看 http://jsoup.org/。
...
![](../images/00014.jpeg)提示:JSON 还有一个构建器——JSON builder。
