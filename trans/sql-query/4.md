...
| ![](../images/00003.gif) | 注意:在计算任何聚合函数时，都会忽略空值。 |
...
| ![](../images/00003.gif) | 注意:不能对包含聚合或子查询的表达式执行聚合函数。 |
...
| ![](../images/00003.gif) | 注意:先不要担心 DatePart 函数。我们将在第 7 章介绍所有的日期和时间功能。 |
...
| ![](../images/00005.gif) | 提示:我们可以添加一个 Group By 子句来获取销售层次结构中任何级别(如组、区域、地区等)的汇总细分。)我们感兴趣的。 |
...
| ![](../images/00005.gif) | 提示:即使很容易被忽略，也要记住 HAVING 子句。需要时，该子句将简化查询语法。 |
...
| ![](../images/00003.gif) | 注意:这个限制的例外是如果指定了 TOP、OFFSET 或 FETCH 子句。我们将很快讨论 OFFSET 和 FETCH。TOP 子句很简单，只需指定要返回的记录数。包含这些子句中的任何一个，排序顺序将影响哪些记录将被返回。 |
...
| ![](../images/00003.gif) | 注意:如果要指定条件排序的方向，请在 case 语句结束后指定。条件排序方式不能更改排序方式的方向，只能更改要使用的列。 |
...
| ![](../images/00003.gif) | 注意:OFFSET 和 FETCH 直到 SQL Server 2012 才添加。 |
