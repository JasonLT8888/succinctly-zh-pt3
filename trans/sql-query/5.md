...
| ![](../images/00005.gif) | 提示:为了确保员工没有重复的电话类型，您可以在员工标识、电话类型组合上添加唯一的约束。但是，如果您已经有许多副本，这可能不是一个选项。 |
...
| ![](../images/00003.gif) | 注意:我们还必须将 PhoneType 检查从 WHERE 子句移到 JOIN 子句，以便将其作为外部连接条件包含在内。 |
...
| ![](../images/00003.gif) | 注意:每当您有像代码清单 5-3 中的子查询时，您必须给它一个别名，以便您可以在查询的其他地方引用它。在这种情况下，您以后不必引用它，但您仍然必须提供一个别名；在这种情况下，p。 |
...
| ![](../images/00003.gif) | 注意:即使我们的表值函数将返回一条记录，它仍然返回一个结果集。 |
...
| ![](../images/00003.gif) | 注意:因为我们在四个单独的 OUTER APPLY 语句中调用这个表值函数，所以对于返回的每个记录，我们都要调用四次。随着记录数量的增长，这可能会变得非常昂贵。 |

如果您没有导致这种性能问题的数据量，那么这个查询可能是您所需要的全部，但是如果您确实遇到了性能问题，那么我们可能需要更有创意地优化这个查询来提高性能。

| ![](../images/00005.gif) | 提示:除非你有性能问题，否则不要优化清晰度。难以遵循的巧妙解决方案将难以维护，并且可能存在难以检测或解决的错误。 |
...
| ![](../images/00005.gif) | 提示:当分解查询时，中间通常是开始的最佳位置。 |
...
| ![](../images/00003.gif) | 注:层次结构表可用于跟踪从组织结构图到销售层次结构、操作层次结构、问卷上的嵌套问题或物料清单的所有层次结构数据。不管存储数据的目的是什么，这些技术中的许多都是有用的。 |
...
| ![](../images/00005.gif) | 提示:定期运行这样的查询，以尽快跟踪潜在的数据完整性问题。如果员工缺少其“报告到”值，我们需要提供一个占位符来进行跟踪。 |
...
| ![](../images/00003.gif) | 注意:每当您多次引用同一个表时，您都需要通过别名为每个引用指定一个唯一的名称。 |
