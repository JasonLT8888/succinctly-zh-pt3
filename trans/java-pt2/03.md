...
| ![](../Images/note.png) | 注意:在 Java 中，子类只能有一个父类。有些语言允许多个父类，称为多重继承，但 Java 只允许一个。然而，Java 确实允许一个子类实现多个接口(参见下一节)。 |
...
| ![](../Images/note.png) | 注意:在 Java 中，不可能从多个父类继承。然而，从多个接口继承是完全合法的。 |
