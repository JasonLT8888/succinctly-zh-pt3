...
| ![](../Images/tip.png) | 提示:始终限制允许客户端发送到应用程序的数据量。 |
...
| ![](../Images/tip.png) | 提示:输入代表新城市的 JSON 对象时要小心。JSON 很简单，但是如果您忘记关闭引号或大括号，或者如果您将引号(表示字符串数据类型)放在预期为整数的值周围，这是不可原谅的。 |
