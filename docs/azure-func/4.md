...
我们可以通过进入我们的网络钩子函数的**集成**部分并创建一个新的**蓝色BLOB存储输出**类型来实现这一点，如下所示。

![](img/image081.jpg)

图 4-r:为网络钩子函数创建一个BLOB输出

确保您指定了正确的**路径**值，将其设置为我们一直使用的**您好请求** blob，并选择正确的**存储帐户连接**。您可以将**BLOB参数名称**保留为默认选项。
